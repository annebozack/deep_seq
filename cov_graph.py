...
def plot(path1, path2, path3, path4, filename):
	calculated1 = getColumn(path1, 2)
	calculated1 = getInt(calculated1)
	cov1 = getColumn(path1, 3)
	cov1 = getInt(cov1)
	calculated2 = getColumn(path2, 2)
	calculated2 = getInt(calculated2)
	cov2 = getColumn(path2, 3)
	cov2 = getInt(cov2)
	calculated3 = getColumn(path3, 2)
	calculated3 = getInt(calculated3)
	cov3 = getColumn(path3, 3)
	cov3 = getInt(cov3)
	calculated4 = getColumn(path4, 2)
	calculated4 = getInt(calculated4)
	cov4 = getColumn(path4, 3)
	cov4 = getInt(cov4)
	calculated = calculated1 + calculated2 + calculated3 + calculated4
	cov = cov1 + cov2 + cov3 + cov4
	slope, intercept, r_value, p_value, std_err = stats.linregress(calculated1, cov1)
	r2 = r_value**2
